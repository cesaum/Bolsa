...
func (b *Book) AddTransaction(transaction *Transaction, wg *sync.WaitGroup) {
	...
	transaction.CalculateTotal(transaction.Shares, transaction.Prince)

	transactions.CloseBuyTransaction()
	transactions.CloseSellTransaction()
	if transaction.BuyingOrder.Pending == 0 {
		transaction.BuyingOrder.Pending.Order.Status = "CLOSED"		
	}
	if transaction.SellingOrder.Pending == 0 {
		transaction.SellingOrder.Pending.Order.Status = "CLOSED"		
	}
